...
class Sales_model extends CI_Model {
	...
	public function delete_sales($ids){
		...
		//=============================get sales_payament db data==================================================
		$this->db->select('account_id,payment');
		$this->db->from('db_salespayments');
		$this->db->where('sales_id', $ids );
		$query = $this->db->get();
		$salespayment_row = $query->row_array();
		if (($salespayment_row['account_id'] != "")){
		$this->db->set('balance', 'balance-'. $salespayment_row['payment'].'',false);
		$this->db->where('id', $salespayment_row['account_id']);
		$res = $this->db->update('ac_accounts');
		}
